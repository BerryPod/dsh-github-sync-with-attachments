/**
 * dsh-github-sync-with-attachments — Browser half.
 *
 * Forked from `dsh-github-sync` 0.4.3 (MIT); the difference the user sees is the
 * `attachments` mirror group, its toggle, and the attachment size in the local
 * stats.
 *
 * One surface: a `settings.section` page. Everything the page shows comes from
 * the plugin's own HTTP API (`/dsh-github-sync-with-attachments/api/…`), which
 * the host half registers on the loopback web server; that route carries no auth
 * of its own, so this file must never receive the access token — the host
 * reports `hasToken` and nothing else.
 *
...
const NS = 'dsh-github-sync-with-attachments'
const CLIENT_NAME = 'dsh-github-sync-with-attachments'
const API = '/dsh-github-sync-with-attachments/api'
/** Lowest host-half API this bundle can drive. */
const REQUIRED_API = 5
/**
 * The mirror groups shown as their own row in the result and compare cards.
 * `settings` is deliberately absent — it is off by default and reported through
 * the toggle, not as a per-run count.
 */
const REPORTED_GROUPS = ['sessions', 'attachments', 'plugins']
/** Copy key naming each group, shared by every per-group row. */
const GROUP_LABEL = {
  sessions: 'tabSessions',
  attachments: 'toggleAttachments',
  plugins: 'togglePlugins',
...
const ZH = {
  title: 'GitHub 同步',
  subtitle: '把本机的会话、附件、插件清单与设置备份到一个私有 GitHub 仓库，并在多台机器之间恢复。',
  tabOverview: '概览',
  tabSessions: '会话备份',
  tabSnapshots: '本地快照',
  tabAdvanced: '高级',
  ...
  toggleAttachments: '附件 attachments',
  toggleAttachmentsHint: '会话里引用的图片与文件对象（$DSH_HOME/attachments）。不开这一项，恢复到别的机器上的会话会因缺图而每次请求都失败',
  togglePlugins: '插件清单',
...
const EN = {
  title: 'GitHub sync',
  subtitle: 'Back up this machine\'s sessions, attachments, plugin manifests and settings to a private GitHub repository, and restore them on another machine.',
  ...
  toggleAttachments: 'Attachments',
  toggleAttachmentsHint: 'The images and files sessions reference ($DSH_HOME/attachments). Without this group a session restored on another machine fails every request — before it ever reaches the provider',
  ...
  return h('div', { className: 'dgs-root' },
    ...
    tab === 'overview' && h('div', { className: 'dgs-panes' },
      h(Card, { title: t('statusTitle') },
        !status
          ...
          : h('div', { className: 'dgs-stats' },
            ...
            h(Stat, { label: t('localAttachments'), value: status.local.attachments ? `${status.local.attachments.files}` : '—' }),
            h(Stat, { label: t('localBytes'), value: formatBytes(status.local.sessions.bytes + (status.local.attachments ? status.local.attachments.bytes : 0)) }),
        ...
        status && status.lastResult
          ? h('div', { className: 'dgs-result' },
            ...
            // Each group is reported apart — "12 changed" would hide whether
            // anything needs installing, or whether the attachments that make a
            // restored session usable came along.
            REPORTED_GROUPS.map((group) => {
              const g = (status.lastResult.groups || {})[group]
              if (!g) return null
              return h('div', { key: group, className: 'dgs-row dgs-wrap' },
                h('span', { className: 'dgs-strong' }, t(GROUP_LABEL[group])),
      ...
      h(Card, { title: t('contentTitle'), hint: t('contentHint') },
        ...
        h(Toggle, { label: t('toggleAttachments'), hint: t('toggleAttachmentsHint'), checked: settings.syncAttachments !== false, onChange: (v) => patch('syncAttachments', v) }),
...
const plugin = {
  ...
  apply(ctx) {
    ...
    try {
      ...
      try {
        console.error('[dsh-github-sync-with-attachments] locale init:', error)
      } catch {
        /* ignore */
      }
    }
