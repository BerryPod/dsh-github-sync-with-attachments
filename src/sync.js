...
/**
 * dsh-github-sync-with-attachments — the sync engine.
 *
 * Forked from `dsh-github-sync` 0.4.3 (MIT) with one addition: the durable
 * attachment store is a first-class mirror group. Everything else — the
 * per-machine repository layout, the content-addressed diff, the restore
 * safety rails — is upstream behaviour.
 *
 ...
 *   instances/<instanceId>/attachments/<version>/…        attachment store
...
import { isExcludedSessionFile, isExcludedAttachmentPath, attachmentDigestOf, PLUGIN_MANIFEST_FILES, displayPath, dshHome, listWorkspaceDirs, listProfiles } from './paths.js'
import { gitBlobSha, repoSlug } from './github.js'

/**
 * The independently switchable mirror groups, in UI order.
 *
 * `attachments` is this fork's addition. It must be listed here, not just
 * handled in one place: {@link fileMissingOnPurpose} reads this array to decide
 * whether a file missing from the remote tree was deleted locally or merely
 * belongs to a group this run does not cover, and the manifest/UI enumerate it.
 */
export const GROUPS = ['sessions', 'attachments', 'plugins', 'settings']
...
export function seedFileBody() {
  return [
    '# dsh 备份仓库',
    '',
    '这个仓库由 dsh 插件 `dsh-github-sync-with-attachments` 维护：把 dsh 的会话、附件、',
    '插件清单与设置备份到私有仓库，并可在另一台机器上按整机 / 工作区 / 单会话恢复。',
    '',
    '```',
    'instances/<实例ID>/',
    '  manifest.json   该机器的清单（主机名、分组、统计、工作区真实路径）',
    '  sessions/…      会话日志，按工作区/会话目录逐字节复制（含各代日志）',
    '  attachments/…   附件存储（$DSH_HOME/attachments，图片与文件对象）',
    '  plugins/…       各 profile 的插件清单（package.json / cordis.patch.yml / 锁文件）',
    '  settings/…      settings.yaml（可选，默认关闭）',
    '```',
    '',
    '`attachments/` 是会话能真正跑起来的前提：会话日志按 sha256 引用附件对象，缺一个',
    '就会让该会话的每一次请求都失败。',
    '',
 ...
 * @param {object} options.groups      `{ sessions, attachments, plugins, settings }` booleans
...
export async function buildPlan({ home = dshHome(), instanceId, groups = {}, profiles, maxFileMb = DEFAULT_MAX_FILE_MB, excludeWorkspaces = [] } = {}) {
  ...
  // The durable attachment store. Without this group a session restored on
  // another machine references image objects that do not exist there, and the
  // harness fails every request before it ever reaches the provider.
  if (groups.attachments !== false) {
    const root = join(home, 'attachments')
    const abs = await walkFiles(root, {
      filter: (name, full) => !isExcludedAttachmentPath(relative(root, full).split(sep).join('/')),
    })
    for (const file of abs) {
      const rel = relative(root, file).split(sep).join('/')
      const size = await fsP.stat(file).then((s) => s.size).catch(() => 0)
      if (size > maxBytes) {
        skipped.push({ path: `${prefix}/attachments/${rel}`, reason: `超过单文件上限 ${maxFileMb} MB（${(size / 1048576).toFixed(1)} MB）` })
        continue
      }
      files.push({ repoPath: `${prefix}/attachments/${rel}`, abs: file, size, group: 'attachments' })
    }
  }
...
export function mapRepoPathToLive(repoPath, home = dshHome()) {
  ...
  if (rest[0] === 'attachments' && rest.length >= 2) {
    return { abs: join(home, 'attachments', ...rest.slice(1)), group: 'attachments' }
  }
...
export function mapLivePathToRepo(abs, { home = dshHome(), instanceId } = {}) {
  ...
  if (rel.startsWith('attachments/')) return `${prefix}/attachments/${rel.slice('attachments/'.length)}`
...
/** Attachment-store path (store-relative) inside a repository path, or null. */
export function attachmentStorePath(repoPath) {
  const parts = String(repoPath).split('/')
  const at = parts.indexOf('attachments')
  if (at < 0 || parts.length < at + 2) return null
  return parts.slice(at + 1).join('/')
}

/**
 * Would writing this file corrupt the local attachment store?
 *
 * Attachment objects are content-addressed: the harness re-hashes the bytes it
 * reads and rejects a mismatch as `ATTACHMENT_CORRUPT`, which then surfaces as
 * the very same opaque provider "stream failed" error this group exists to
 * prevent. A truncated upload, a text-mode round trip or a hand-edited backup
 * would all land that failure, so a restore verifies the digest promised by the
 * path before publishing the file. Paths that are not content-addressed (the
 * `files/…/<name>` alias is addressed by its parent digest, which we do check)
 * are accepted as-is.
 *
 * @returns a human-readable reason to skip, or `null` when the bytes are fine.
 */
export function invalidAttachmentReason(repoPath, buffer) {
  const storePath = attachmentStorePath(repoPath)
  if (storePath === null) return null
  const expected = attachmentDigestOf(storePath)
  if (expected === undefined) return null
  if (!Buffer.isBuffer(buffer)) return '附件内容不是字节流'
  const actual = createHash('sha256').update(buffer).digest('hex')
  if (actual !== expected) {
    return `内容 sha256 与路径承诺的不一致（路径 ${expected.slice(0, 12)}…，实际 ${actual.slice(0, 12)}…）；写入它会得到 ATTACHMENT_CORRUPT`
  }
  return null
}

/** The store publishes every durable object read-only; mirror that on restore. */
function attachmentFileMode(repoPath) {
  // 0o400 is what `dsh-attachment-local` chmods a published object to (its
  // request-image cache uses 0o600, but that tree is never mirrored).
  return attachmentStorePath(repoPath) === null ? undefined : 0o400
}
...
export async function restoreFrom({ source, home = dshHome(), only = {}, overwrite = true, dryRun = false, allowSettings = false, workspaceMap = {}, onProgress } = {}) {
  ...
  for (const { entry, target } of planned) {
    ...
    const corruptAttachment = invalidAttachmentReason(entry.path, buffer)
    if (corruptAttachment !== null) {
      skipped.push({ path: entry.path, reason: `附件校验失败：${corruptAttachment}；已跳过` })
      continue
    }
    ...
    // Restored attachment objects keep the store's read-only convention, so a
    // later publish of the same digest hits the immutable-object path instead of
    // rewriting bytes under a hash that no longer describes them.
    const mode = attachmentFileMode(entry.path)
    if (mode !== undefined) await fsP.chmod(target.abs, mode).catch(() => {})
