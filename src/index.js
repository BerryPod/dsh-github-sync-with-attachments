...
import { dshHome, displayPath, decodeWorkspaceKey, isExcludedAttachmentPath, listProfiles, listWorkspaceDirs, workspacePathMap } from './paths.js'
...
const NAME = 'dsh-github-sync-with-attachments'
const SETTINGS_NS = 'dsh-github-sync-with-attachments'
const API_PREFIX = `/${NAME}/api`

/**
 * The original plugin, whose per-machine id and JSON config file this fork
 * adopts on first run so an existing backup keeps its `instances/<id>/` folder
 * (and its restore history) instead of starting a second, parallel one.
 */
const LEGACY_NAME = 'dsh-github-sync'
 ...
 * speaks.
 *
 * 5: the `attachments` mirror group exists and `/status` reports the local
 * attachment store.
 */
const API_VERSION = 5
...
const DEFAULTS = {
  ...
  syncSessions: true,
  /**
   * This fork's reason to exist, so it defaults on. Sessions without their
   * attachments still restore, but every request in them fails locally before
   * reaching the provider — a repaired-looking session that cannot be used.
   */
...
const BOOLEAN_KEYS = ['syncSessions', 'syncAttachments', 'syncPlugins', 'syncSettings', 'autoSync', 'syncOnStartup', 'pullRequest', 'snapshotBeforePush']
...
/**
 * Count what the attachment mirror would upload.
 *
 * The store is the one group whose size is not visible anywhere else in the UI,
 * and a first push of a long-lived installation can be tens of megabytes — the
 * user deserves to see that number before pressing the button.
 */
async function localAttachmentInventory(home = dshHome()) {
  const root = join(home, 'attachments')
  let files = 0
  let bytes = 0
  for (const file of await walkFiles(root)) {
    const storeRel = relative(root, file).split(sep).join('/')
    if (isExcludedAttachmentPath(storeRel)) continue
    const stat = await fsP.stat(file).catch(() => null)
    if (!stat) continue
    files += 1
    bytes += stat.size
  }
  return { files, bytes, path: displayPath(root) }
}
...
export function apply(ctx, config = {}) {
  ...
  // The original plugin's own state directory. Read-only here: it is where an
  // installation that upgraded to this fork keeps its machine id and its JSON
  // fallback config, so the fork can continue the same `instances/<id>/`
  // backup instead of orphaning the one already in the repository.
  const legacyDir = join(home, LEGACY_NAME)
  ...
  const baseSettings = () => {
    ...
    // One-time migration, lowest precedence: an installation that upgraded from
    // the original plugin keeps its repository and token, and the composition
    // layer (`config`) still wins.
    const legacy = readLegacySettings({
      home,
      booleans: new Set(BOOLEAN_KEYS),
      numbers: new Set(NUMBER_KEYS),
      strings: new Set(STRING_KEYS),
    })
    for (const key of Object.keys(base)) if (legacy[key] !== undefined) base[key] = legacy[key]
        ...
        try {
          settingsPlug.scope = svc.register(
            ...
            Schema.object({
              ...
              syncAttachments: Schema.boolean().default(true),
  ...
  const groupToggles = (settings) => ({
    ...
    attachments: settings.syncAttachments !== false,
  ...
  const stateLoaded = fsP
    ...
    .catch(() => {
      // This fork's first run on a machine that already backed up with the
      // original plugin: adopt its machine id, so the repository keeps exactly
      // one `instances/<id>/` directory per machine and the existing session
      // backups stay restorable from this UI.
      return fsP
        .readFile(join(legacyDir, 'state.json'), 'utf8')
        .then(async (raw) => {
          const legacy = JSON.parse(raw)
          if (legacy && typeof legacy.instanceId === 'string' && legacy.instanceId !== '') {
            state.instanceId = legacy.instanceId
            // Persist it immediately: from here on this directory owns the id, so
            // deleting the original plugin (or its state) can never mint a second
            // machine id and orphan the existing backup in the repository.
            await atomicWriteFile(stateFile, JSON.stringify(state, null, 2)).catch(() => {})
            const note = `沿用原插件 dsh-github-sync 的实例 ID：${legacy.instanceId}`
            if (typeof log.info === 'function') log.info(note)
            else log.warn(note)
          }
        })
        .catch(() => {})
    ...
    syncRun = (async () => {
      ...
      try {
        ...
        const result = {
          ...
          pr: report.pr || null,
          // Sessions, attachments and plugins are reported apart: they are
          // different kinds of thing to act on, and a single "12 changed"
          // number hides which.
        ...
        handler: async (req, res) => {
          ...
          try {
            ...
            if (method === 'GET' && route === `${API_PREFIX}/status`) {
              ...
              const attachments = await localAttachmentInventory(home)
            ...
            if (method === 'POST' && route === `${API_PREFIX}/pull`) {
              ...
              const only = { instanceId, groups: Array.isArray(body.groups) && body.groups.length ? body.groups : ['sessions', 'attachments', 'plugins'] }
            ...
            if (method === 'POST' && route === `${API_PREFIX}/sessions/restore`) {
              ...
              // The attachment store is machine-global rather than per-workspace,
              // so a workspace restore brings all of it along: dragging along a
              // few extra immutable objects is harmless, while a restored session
              // missing one of its images cannot send a single request.
              const withAttachments = body.attachments !== false
              const only = { instanceId, groups: withAttachments ? ['sessions', 'attachments'] : ['sessions'] }
              ...
              try {
                ...
                watch.phase('拍本地快照')
                const plan = await buildPlan({ home, instanceId: await ensureInstanceId(), groups: { sessions: true, attachments: true, plugins: false, settings: false } })
